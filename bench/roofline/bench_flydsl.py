...
from bench.roofline.capture import EmpiricalRoofline, load_empirical_roofline
from bench.roofline.regime import CeilingType, classify_regime, Regime
...
def _quantize_fp8_row(x: torch.Tensor):
    ...
    scale = xmax / torch.finfo(torch.float8_e4m3fnuz).max
    xq = (x / scale).to(torch.float8_e4m3fnuz)
...
def bench_flydsl_preshuffle(
    ...
    scale_bytes = (x_scale.numel() + w_scale.numel()) * 4  # FP32 scales
...
@click.option("--shapes", default=None, help="Preset: llama3_70b, llama3_405b, roofline_validation")
def main(
    roofline_json, target_template, output_dir, m_vals, n_vals, k_vals,
    stat_iterations, device, shapes,
):
    ...
    if shapes == "llama3_70b":
        ...
        shape_list = [
            (8192, 8192, 8192), (4096, 4096, 4096), (1024, 8192, 8192),
            (128, 8192, 8192), (16, 8192, 8192), (1, 8192, 8192),
        ]
    else:
    ...
    if roofline:
        print(f"Empirical roofline: FP8 MFMA={roofline.mfma_peak_tflops.get('fp8', 0):.1f} TFLOPS, "
              f"HBM={roofline.hbm_bw_gbps:.1f} GB/s")
    print(f"Benchmarking {len(shape_list)} shapes with {stat_iterations} iterations each\n")

    # Header
    print(f"{'Shape':<25} {'Regime':<6} {'Ms':<10} {'TFLOPS':<10} {'GB/s':<10} "
    ...
    for M, N, K in shape_list:
        try:
            r = bench_flydsl_preshuffle(
                M, N, K, roofline, target_matrix,
                n_iterations=stat_iterations, device=device,
            )
            pass_str = "PASS" if r.target_pass else ("FAIL" if r.target_pass is False else "")
            ceiling_str = f"{r.ceiling_type}={r.ceiling_value:.0f}"
            print(
                f"({r.M},{r.N},{r.K}){'':<{25-len(f'({r.M},{r.N},{r.K})')}} "
    ...
    if results:
        ...
        rows = [{
            "kernel": r.kernel, "M": r.M, "N": r.N, "K": r.K,
            "median_ms": r.median_ms, "tflops": r.tflops, "gbps": r.gbps,
            "regime": r.regime, "ceiling_type": r.ceiling_type,
            "ceiling_value": r.ceiling_value, "achieved_pct": r.achieved_pct,
            "target_pct": r.target_pct, "target_pass": r.target_pass,
            "cov_pct": r.cov_pct, "n_iters": r.n_iters,
        } for r in results]
