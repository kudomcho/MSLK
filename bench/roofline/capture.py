    ...
    try:
        ...
        for line in r.stdout.splitlines():
            if "gfx9" in line and "Name:" in line:
                arch = line.split(":")[-1].strip().split(":")[0]
                break
        return arch
    except Exception:
        return ""
...
def parse_roofline_csv(csv_path: str, device_id: int = 0) -> dict:
    ...
    headers = rows[0][1:]
    values = rows[1 + device_id][1:]

    data = {h: float(v) for h, v in zip(headers, values) if v}
        ...
        if not os.path.exists(csv_path):
            ...
            if candidates:
                ...
                raise FileNotFoundError(
                    f"roofline.csv not found in {workload_dir}"
                )

        parsed = parse_roofline_csv(csv_path, device_id=0)
