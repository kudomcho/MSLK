...
from mslk.bench.common.utils import (
    BenchOptions,
    BenchStatResult,
    common_bench_options,
    do_bench_statistical,
    profiler,
)
...
from bench.roofline.targets import load_target_matrix, lookup_target, TargetMatrix
...
class Metrics:
    ...
    def __str__(self) -> str:
        ...
        achieved_str = f"{self.achieved_pct:<10.2f}" if self.achieved_pct > 0 else f"{'':10}"
...
def _enrich_metrics_with_regime(
    ...
    """Add regime classification, ceiling, achieved %, and target pass/fail to metrics."""
    ...
    if rc.ceiling_type == CeilingType.MFMA_PEAK and compute_roofline_tflops:
        metrics.ceiling_type = "mfma_peak"
        ...
        metrics.ceiling_type = "empirical_best"
    ...
    if _target_matrix is not None:
        ...
        if target is not None:
            ...
            if metrics.achieved_pct > 0:
                metrics.target_pass = metrics.achieved_pct >= target.target_pct
    elif metrics.achieved_pct > 0:
...
def benchmark(
    ...
    for gemm_op in gemm_ops:
        ...
        # For cold-L2 on memory-bound shapes, override rotating_buffer
        bench_opts = opts
        if _cold_l2:
            rc = classify_regime(m, n, k)
            if rc.regime == Regime.R3_SKINNY_DECODE:
        ...
        with profiler(enabled=opts.trace, with_stack=True):
            if _stat_iterations > 1:
                stat = do_bench_statistical(
                    gemm_op.compute,
                    quantized_vals,
                    bench_opts,
                    n_iterations=_stat_iterations,
                )
                ms_runtime = stat.median_ms
                ...
                if stat.cov_pct > _cov_bound:
                    print(
                        ...
                        f"{_cov_bound}% for {gemm_op.name} ({m},{n},{k})"
                    )
            else:
...
@click.option(
    "--cold-l2/--no-cold-l2",
    default=False,
    help="Force rotating buffer for memory-bound R3 shapes.",
)
@click.option(
    "--target-matrix",
    "target_matrix_path",
...
def invoke_main(
    ...
    if target_matrix_path:
        ...
        print(f"Loaded target matrix from {target_matrix_path} ({len(matrix.targets)} targets)")
    ...
    if stat_iterations > 1:
        print(f"    Statistical iterations: {stat_iterations} (CoV bound: {cov_bound}%)")
    if cold_l2:
